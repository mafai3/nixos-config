...
# 1.3 เปลี่ยน Shell เป็น Zsh เพื่อให้มีสีตอนพิมพ์คำสั่ง
 ...
 environment.shellAliases = {
  ...
  # ตั้งค่าให้ i3 เป็น default session
  ...
  # security.rtkit.enable = true;      # ปิดถ้าไม่ทำ audio production (ประหยัด ~20MB)
  services.pipewire = {
    enable = true;
    alsa.enable = true;
  ...
  environment.systemPackages = with pkgs; [
    ...
    flameshot
    lxqt.lxqt-policykit  # เบากว่า polkit_gnome (~50MB)
    fastfetch
    veracrypt       # ปิดถ้าไม่ใช้ encryption
    picom
    feh
    git
